...
exports.getQuizForEdit = async (req, res) => {
  try {
    ...
    for (const q of questions) {
      const [options] = await db.query('SELECT option_text, option_index FROM options WHERE question_id = ? ORDER BY option_index', [q.id]);
...
exports.updateQuiz = async (req, res) => {
  try {
    ...
    await db.query('DELETE FROM options WHERE question_id IN (SELECT id FROM questions WHERE quiz_id = ?)', [req.params.id]);
    await db.query('DELETE FROM questions WHERE quiz_id = ?', [req.params.id]);
...
exports.deleteQuiz = async (req, res) => {
  try {
    await db.query('DELETE FROM quizzes WHERE id = ? AND creator_id = ?', [req.params.id, req.user.id]);
