...
export default function MyQuizzes() {
  ...
  const loadQuizzes = useCallback(async () => {
    ...
    try {
      ...
      setError(err?.response?.data?.message || 'Ошибка загрузки квизов');
  ...
  const loadResults = useCallback(async (quizId = '') => {
    ...
    try {
      ...
      const res = await api.get(`/creator/results${params}`);
      setResults(Array.isArray(res.data) ? res.data : []);
    } catch (err) {
      setError(err?.response?.data?.message || 'Ошибка загрузки результатов');
    } finally { setResLoading(false); }
  }, []);

  useEffect(() => { loadQuizzes(); }, [loadQuizzes]);
  useEffect(() => { if (activeTab === 'results') loadResults(filterId); }, [activeTab, filterId, loadResults]);
  ...
  const handleExport = async () => {
    setExporting(true);
    try {
      const params = filterId ? `?quizId=${filterId}` : '';
      const res = await api.get(`/creator/export-results${params}`, { responseType: 'blob' });
      const url  = window.URL.createObjectURL(new Blob([res.data]));
      const link = document.createElement('a');
      link.href  = url;
      link.setAttribute('download', 'my-quiz-results.xlsx');
  ...
  return (
    <div style={page}>
      <div style={container}>
        ...
        <div style={tabBar}>
          {TABS.map(t => (
            <button key={t.id} style={activeTab === t.id ? tabActive : tab} onClick={() => setActiveTab(t.id)}>
              {t.label}
            </button>
          ))}
        </div>
        ...
        {activeTab === 'results' && (
          <div>
            <div style={resultsToolbar}>
              <select style={filterSelect} value={filterId} onChange={e => setFilterId(e.target.value)}>
            ...
            {resLoading && <div style={emptyBox}>Загружаем результаты…</div>}

            {!resLoading && results.length === 0 && (
              <div style={emptyBox}>
                <div style={{ fontSize: 48, marginBottom: 12 }}>📭</div>
                <p style={{ color: '#9ca3af', margin: 0 }}>Пока нет результатов</p>
              </div>
            )}
            ...
            {!resLoading && results.length > 0 && (
              <div style={tableWrap}>
                <table style={table}>
                  <thead>
                    <tr>
                      {['Участник', 'Email', 'Квиз', 'Балл', 'Результат', 'Сдано', 'Нарушений', 'Дата'].map(h => (
                  ...
                  <tbody>
                    {results.map((r, i) => (
                      <tr key={r.id} style={{ background: i % 2 === 0 ? 'transparent' : 'rgba(255,255,255,0.02)' }}>
                        ...
                        <td style={{ ...td, color: '#9ca3af', fontSize: 12 }}>{r.email}</td>
                        <td style={td}>{r.quiz_title}</td>
                        <td style={{ ...td, textAlign: 'center' }}>{r.score}/{r.max_score}</td>
                        <td style={{ ...td, textAlign: 'center' }}>
                          <span style={{
                            padding: '3px 10px', borderRadius: 99, fontSize: 12, fontWeight: 700,
                            background: Number(r.percent_score) >= 60 ? 'rgba(16,185,129,0.15)' : 'rgba(239,68,68,0.15)',
                            color: Number(r.percent_score) >= 60 ? '#10B981' : '#EF4444',
                          }}>
                            {Number(r.percent_score).toFixed(0)}%
                          </span>
                        </td>
                        <td style={{ ...td, textAlign: 'center' }}>{r.is_passed ? '✅' : '❌'}</td>
                        <td style={{ ...td, textAlign: 'center' }}>
                          {r.violations_count > 0
                            ? <span style={{ color: '#F59E0B', fontWeight: 700 }}>⚠️ {r.violations_count}</span>
...
function QuizCard({ quiz, onToggle, onDelete }) {
  ...
  const handleToggle = async () => { setToggling(true); await onToggle(quiz); setToggling(false); };
  ...
  return (
    <div style={card}>
      <div style={coverWrap}>
        ...
        <div style={{ ...statusBadge, background: quiz.is_published ? 'rgba(16,185,129,0.2)' : 'rgba(124,58,237,0.2)', color: quiz.is_published ? '#10B981' : '#a78bfa', borderColor: quiz.is_published ? 'rgba(16,185,129,0.3)' : 'rgba(124,58,237,0.3)' }}>
      ...
      <div style={cardActions}>
        <button onClick={handleToggle} disabled={toggling} style={quiz.is_published ? unpublishBtn : publishBtn}>
...
const tabBar         = { display: 'flex', gap: 8, marginBottom: 28, borderBottom: '1px solid rgba(255,255,255,0.08)' };
const tabBase        = { padding: '10px 20px', border: 'none', borderRadius: '10px 10px 0 0', fontWeight: 700, fontSize: 14, cursor: 'pointer' };
const tab            = { ...tabBase, background: 'transparent', color: '#6b7280' };
const tabActive      = { ...tabBase, background: 'rgba(124,58,237,0.15)', color: '#c4b5fd', borderBottom: '2px solid #7C3AED' };
const resultsToolbar = { display: 'flex', gap: 12, alignItems: 'center', marginBottom: 20, flexWrap: 'wrap' };
const filterSelect   = { padding: '9px 14px', borderRadius: 10, border: '1px solid rgba(255,255,255,0.12)', background: 'rgba(255,255,255,0.05)', color: '#f4f7fb', fontSize: 14, minWidth: 200, cursor: 'pointer' };
const exportBtn      = { padding: '9px 18px', borderRadius: 10, border: 'none', background: 'linear-gradient(135deg,#7C3AED,#14B8A6)', color: '#fff', fontWeight: 700, fontSize: 14, cursor: 'pointer' };
const tableWrap      = { overflowX: 'auto', borderRadius: 16, border: '1px solid rgba(255,255,255,0.08)' };
const table          = { width: '100%', borderCollapse: 'collapse', fontSize: 13 };
const th             = { padding: '12px 14px', textAlign: 'left', fontWeight: 700, fontSize: 11, color: '#6b7280', textTransform: 'uppercase', letterSpacing: '0.06em', borderBottom: '1px solid rgba(255,255,255,0.08)', background: 'rgba(255,255,255,0.03)', whiteSpace: 'nowrap' };
const td             = { padding: '11px 14px', color: '#e2e8f0', borderBottom: '1px solid rgba(255,255,255,0.04)', verticalAlign: 'middle' };
